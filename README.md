...
0. Alternatively start a bash prompt in your container and from there you can modify the image defaults or run your cmds
`docker-compose run --service-ports --rm panoptes bash`
And from within the container run the cmds you want
`/rails_app/scripts/docker/start.sh` or
`rake db:create`, `bundle exec rspec`, `bundle exec puma`, etc.
